...
export function Favorite({ size = 'm', className = '', ariaLabel = 'Save restaurant', onClick }: FavoriteProps) {
  const classNames = ['thumbnail-favorite', `thumbnail-favorite--${size}`, className].filter(Boolean).join(' ')
  ...
  return (
    <div className={classNames} aria-hidden data-name="[Eater}-save-btn" data-node-id="79589:184146">
      {heart}
    </div>
  )
}
