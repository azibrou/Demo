...
export function ShoppingListScreen() {
  ...
  const shoppingBasketUnits = useMemo(
    () => items.reduce((acc, r) => acc + (r.inBasket ? r.basketQty : 0), 0),
    [items],
  )

  const initialBasketLayoutDone = useRef(false)
  const lastSyncedShoppingSum = useRef<number | null>(null)

  useLayoutEffect(() => {
    if (initialBasketLayoutDone.current) return
    initialBasketLayoutDone.current = true
    lastSyncedShoppingSum.current = shoppingBasketUnits
    basketFab.syncShoppingListBasketUnits(shoppingBasketUnits, { initial: true })
  }, [basketFab, shoppingBasketUnits])

  useEffect(() => {
    if (lastSyncedShoppingSum.current === null) return
    if (lastSyncedShoppingSum.current === shoppingBasketUnits) return
    lastSyncedShoppingSum.current = shoppingBasketUnits
    basketFab.syncShoppingListBasketUnits(shoppingBasketUnits)
  }, [basketFab, shoppingBasketUnits])
  ...
  return (
    <div className="font-sans min-h-svh w-full bg-white text-[#191f1c]">
      <div className="mx-auto flex min-h-svh w-full max-w-full flex-col sm:max-w-[375px]">
        ...
        {!isEmpty && (
          <div className="fixed left-0 right-0 z-30 w-full border-t border-[rgba(0,45,30,0.07)] bg-white px-6 pb-10 pt-3 sm:left-1/2 sm:max-w-[375px] sm:-translate-x-1/2 bottom-[calc(88px+env(safe-area-inset-bottom,0px))]">
